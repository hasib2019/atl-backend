...
export class Paginate {
  ...
  protected currentPageValid() {
    this.pageValid = this.currentPage <= this.pages && this.currentPage > 0 ? true : false;
  }
...
export function emptyPaginationResponse(page?: number, limit?: number): IPaginationResponse {
  return {
    limit: limit ?? -1,
    totalPages: -1,
    currentPage: page ?? -1,
